...
int sftp_get_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (result &&
		...
		fzprintf(sftpError, "%s: Cannot create directory", outfname);
		return 0;
	    }
	    ...
	    if (!dirhandle) {
		fzprintf(sftpError, "%s: unable to open directory: %s",
		       fname, fxp_error());
		return 0;
	    }
	    nnames = namesize = 0;
		...
		if (names == NULL) {
			...
			break;
		    fzprintf(sftpError, "%s: reading directory: %s", fname, fxp_error());
		    sfree(ournames);
		    return 0;
		}
		if (names->nnames == 0) {
		...
		for (i = 0; i < names->nnames; i++)
			...
			if (!vet_filename(names->names[i].filename)) {
			    fzprintf(sftpStatus, "ignoring potentially dangerous server-"
				   "supplied filename '%s'",
...
int sftp_put_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (!result ||
	    ...
	    if (!result) {
		fzprintf(sftpError, "%s: create directory: %s",
		       outfname, fxp_error());
		return 0;
	    }
	}
	...
	if (!dh) {
	    fzprintf(sftpError, "%s: unable to open directory", fname);
	    return 0;
	}
	...
	if (!xfer_done(xfer)) {
	    ...
	    if (ret <= 0 && !err) {
		fzprintf(sftpError, "error while writing: %s\n", fxp_error());
		err = 1;
	    }
	}
    }
...
SftpWildcardMatcher *sftp_begin_wildcard_matching(char *name)
{
	...
	fzprintf(sftpError, "Unable to open %s: %s", cdir, fxp_error());
	swcm = NULL;
	sfree(unwcdir);
    }
...
char *sftp_wildcard_get_filename(SftpWildcardMatcher *swcm)
{
	...
	if (!swcm->names) {
	    ...
	    if (!swcm->names) {
		if (fxp_error_type() != SSH_FX_EOF)
		    fzprintf(sftpError, "%s: reading directory: %s", swcm->prefix,
			   fxp_error());
		return NULL;
	    }
	...
	if (!vet_filename(name->filename)) {
	    fzprintf(sftpStatus, "ignoring potentially dangerous server-"
		   "supplied filename '%s'", name->filename);
...
int sftp_cmd_ls(struct sftp_command *cmd)
{
	 ...
	 */
	for (i = 0; i < nnames; ++i) {
	    unsigned long long mtime = ((unsigned long long)ournames[i]->attrs.mtime.hi << 32) + ournames[i]->attrs.mtime.lo;
...
static int sftp_cmd_lcd(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 2) {
	fzprintf(sftpError, "lcd: expects a local directory name");
	return 0;
    }

    errmsg = psftp_lcd(cmd->words[1]);
    if (errmsg) {
	fzprintf(sftpError, "lcd: unable to change directory: %s", errmsg);
	sfree(errmsg);
	return 0;
    }

    currdir = psftp_getcwd();
    fzprintf(sftpReply, "New local directory is %s", currdir);
    sfree(currdir);
...
static int sftp_cmd_lpwd(struct sftp_command *cmd)
{
    ...
    currdir = psftp_getcwd();
    fzprintf(sftpReply, "Current local directory is %s", currdir);
    sfree(currdir);
