...
static Loaded_keyfile_list* load_keyfiles(Ssh ssh, int req_type)
{
    ...
    char* val, *file;
    for (val = conf_get_str_strs(ssh->conf, CONF_fz_keyfiles, NULL, &file);
	 val != NULL;
	...
	if (type == SSH_KEYTYPE_SSH1) {
	    ...
	    if (!rsakey_pubblob(loaded->file, &loaded->publickey_blob, &loaded->publickey_bloblen, 0, &error)) {
		if (!error) {
		    error = "unknown";
		}
			 ...
			 struct Packet *pktin)
{
    ...
    /*fflush(stdout); FIXME eh?YEAH fz! */
    {
	...
	s->loaded_keyfile_list = 0;
    } else {
	s->tried_publickey = s->tried_agent = 0;
	/* FZ: Load the keyfiles */
	s->loaded_keyfile_list = load_keyfiles(ssh, SSH_KEYTYPE_SSH1);
    }
	...
	if (conf_get_int(ssh->conf, CONF_tryagent) && agent_exists() && !s->tried_agent) {
		...
		for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
		    ...
		    remove_duplicate_keys(ssh, &s->loaded_keyfile_list, pkblob, pkbloblen);
			     ...
			     struct Packet *pktin)
{
  ...
  begin_key_exchange:
    ...
    if (ssh->kex->main_type == KEXTYPE_DH) {
        ...
        fzprintf(sftpKexAlgorithm, "ECDH");
        fzprintf(sftpKexHash, ssh->kex->hash->text_name);
        fzprintf(sftpKexCurve, ssh_ecdhkex_curve_textname(ssh->kex));
    ...
    fzprintf(sftpMacClientToServer, ssh->csmac_etm ? "%s (in ETM mode)" : "%s", ssh->csmac->text_name);
    ...
    fzprintf(sftpMacServerToClient, ssh->scmac_etm ? "%s (in ETM mode)" : "%s", ssh->scmac->text_name);
...
//	char *username;
    ...
    /* FZ: Load the keyfiles */
    s->loaded_keyfile_list = load_keyfiles(ssh, SSH_KEYTYPE_SSH2);
