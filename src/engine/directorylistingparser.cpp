...
struct cmp
{
	bool operator()(CRefcountObject<wxString> const& lhs, CRefcountObject<wxString> const& rhs)
	{
		return false;
	}

	bool operator()(CRefcountObject<wxString> const& lhs, wxString const& rhs)
	{
		return false;
	}
};
...
bool CDirectoryListingParser::ParseAsUnix(CLine *pLine, CDirentry &entry, bool expect_date)
{
	...
	entry.permissions = cache.get(permissions);
	...
	do
	{
		...
		entry.ownerGroup = cache.get(ownerGroup);
		return true;
	}
	while (numOwnerGroup--);
...
bool CDirectoryListingParser::ParseAsDos(CLine *pLine, CDirentry &entry)
{
	...
	entry.target.clear();
	entry.ownerGroup = cache.get(wxString());
	entry.permissions = cache.get(wxString());
	entry.time += m_timezoneOffset;
...
bool CDirectoryListingParser::ParseAsEplf(CLine *pLine, CDirentry &entry)
{
	...
	while (fact < pos)
	{
		...
		else if (type == 'm')
		{
			...
			entry.permissions = cache.get(token.GetString().Mid(fact + 2, len - 2));
...
bool CDirectoryListingParser::ParseAsIbm(CLine *pLine, CDirentry &entry)
{
	...
	entry.ownerGroup = cache.get(token.GetString());
...
bool CDirectoryListingParser::ParseOther(CLine *pLine, CDirentry &entry)
{
	...
	if (token.IsNumeric()) {
		entry.permissions = cache.get(firstToken.GetString());
...
bool CDirectoryListingParser::ParseAsOS9(CLine *pLine, CDirentry &entry)
{
	...
	entry.permissions = cache.get(token.GetString());
