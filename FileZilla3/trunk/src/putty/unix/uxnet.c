...
static int try_connect(Actual_Socket sock)
{
	...
	(void)setsockopt(s, SOL_SOCKET, SO_RCVBUF, (const char*)&size_read, sizeof(size_read));
    }

    {
	int size_write = 262144;
	(void)setsockopt(s, SOL_SOCKET, SO_SNDBUF, (const char*)&size_write, sizeof(size_write));
    }
